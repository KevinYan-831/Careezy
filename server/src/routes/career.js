...
router.get('/resume-enhancement', authenticateToken, async (req, res) => {
  try {
    const resume = await Resume.findOne({ userId: req.user._id });
    if (!resume) {
      return res.status(404).json({ message: 'Resume not found. Please create a resume first.' });
...
router.post('/skill-recommendations', authenticateToken, async (req, res) => {
  try {
...
router.get('/gap-analysis', authenticateToken, async (req, res) => {
  try {
...
router.get('/industry-insights/:field', authenticateToken, async (req, res) => {
  try {
